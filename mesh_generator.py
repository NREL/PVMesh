...
def set_length_scale(surface_id,  lcmin, lcmax, dismin, distmax ):
    ...
    gmsh.model.mesh.field.setNumbers(
        distance, "FacesList", domain_markers[surface_id]["gmsh_tags"]     # field of distance to suface with tage "gmsh_tags"
    )
    ...
    )                                  # when distance to the surface >  distmax, mesh size=lcmax, if when distance to the surface < distmin, mesh size=lcmin
                                        # between distmax and istmin, mesh size is linearly interpolated between lcmin and lcmax
...
front_encap_thick = 0.45/1000-cell_thick/2    # thickness of front encapsulant layer, in m
back_encap_thick = 0.45/1000-cell_thick/2     # thickness of back encapsulant layer, in m
...
clip_thick = 6.0/1000            # open space of frame (parameter d, d-2e is panel thickness)
...
cell_layer_encap_tag = [x[1] for x in whole_cell_layer if x not in cell_list]  # tag of eva domain in cell layer
...
all_volume_list = [[final_left_frame_tag,final_front_frame_tag,final_right_frame_tag,final_rear_frame_tag], back_sheet_tags, back_encap_tags, front_encap_tags, front_glass_tags]
...
# [1]: volume tages of back sheet
# [2]: volume tages of back encap
# [3]: volume tages of front encap
# [4]: volume tages of front glass
# [5]: volume tages of all cells
# [6]: volume tages of encapsulant in cell layer, [6][0] is the uncovered part, [6][1] is the covered part
# [7]: volume tages of seal layer
...
count_surface =  surface_tags(surf_tag_list,count_surface)

structure_vol_list =["frm"]  # frm0 is left frame, frm1 is front frame, frm2 is right frame, frm3 is rear frame. 
...
"""
# ######################################
# threshold field based ion distance (not using here)
# ######################################


# # # # min_dist = []
# # # # for volid in structure_vol_list[0:4]:   # frames
...
# # # # gmsh.model.mesh.field.setNumbers(minimum, "FieldsList", min_dist)  # take the minimum value of a list of fields.
# # # # gmsh.model.mesh.field.setAsBackgroundMesh(minimum)               # set the minimum field as the background mesh size field
...
gmsh.model.mesh.field.setNumber(volume_field_whole_domain, "VIn", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_whole_domain, "VOut", 3*frame_thick)
...
# define box field for the back sheet domain
volume_field_back_sheet = gmsh.model.mesh.field.add("Box")
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "Thickness", back_sheet_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "VIn", 3*back_sheet_thick)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "VOut", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "XMax", panel_length-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "XMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "YMax", panel_width-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "YMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "ZMax", back_sheet_thick)
gmsh.model.mesh.field.setNumber(volume_field_back_sheet, "ZMin", 0)
box_field_list.append(volume_field_back_sheet)

# define box field for the back EVA domain
volume_field_back_eva = gmsh.model.mesh.field.add("Box")
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "Thickness", back_encap_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "VIn", 3*back_encap_thick)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "VOut", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "XMax", panel_length-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "XMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "YMax", panel_width-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "YMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "ZMax", back_sheet_thick+back_encap_thick)
gmsh.model.mesh.field.setNumber(volume_field_back_eva, "ZMin", back_sheet_thick)
box_field_list.append(volume_field_back_eva)

# define box field for the cell layer domain
volume_field_cell_layer = gmsh.model.mesh.field.add("Box")
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "Thickness", cell_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "VIn", 3*cell_thick)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "VOut", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "XMax", panel_length-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "XMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "YMax", panel_width-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "YMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "ZMax", back_sheet_thick+back_encap_thick+cell_thick)
gmsh.model.mesh.field.setNumber(volume_field_cell_layer, "ZMin", back_sheet_thick+back_encap_thick)
box_field_list.append(volume_field_cell_layer)

# define box field for the front eva domain
volume_field_front_eva = gmsh.model.mesh.field.add("Box")
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "Thickness", front_encap_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "VIn", 3*back_encap_thick)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "VOut", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "XMax", panel_length-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "XMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "YMax", panel_width-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "YMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "ZMax", back_sheet_thick+back_encap_thick+cell_thick+front_encap_thick)
gmsh.model.mesh.field.setNumber(volume_field_front_eva, "ZMin", back_sheet_thick+back_encap_thick+cell_thick)
box_field_list.append(volume_field_front_eva)

# define box field for the front glass domain
volume_field_front_glass = gmsh.model.mesh.field.add("Box")
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "Thickness", front_glass_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "VIn", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "VOut", 3*frame_thick)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "XMax", panel_length-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "XMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "YMax", panel_width-cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "YMin", cover_length)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "ZMax", panel_thick)
gmsh.model.mesh.field.setNumber(volume_field_front_glass, "ZMin", back_sheet_thick+back_encap_thick+cell_thick+front_encap_thick)
box_field_list.append(volume_field_front_eva)

# # # # define box field for uncovered panel part
# # # uncovered_volume_field = gmsh.model.mesh.field.add("Box")
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "Thickness", frame_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "VOut", frame_thick)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "VIn",  frame_thick)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "XMin", cover_length)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "XMax", panel_length-2*cover_length)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "YMin", cover_length)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "YMax", panel_width-2*cover_length)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "ZMax", panel_thick)
# # # gmsh.model.mesh.field.setNumber(uncovered_volume_field, "ZMin", 0)
# # # box_field_list.append(uncovered_volume_field)

# # # # define box field for cells, box will go across whole thickness of panel
# # # for i in range(n_cell_length):
# # #      for j in range(n_cell_width):
# # #         cell_volume_field = gmsh.model.mesh.field.add("Box")
# # #         gmsh.model.mesh.field.setNumber(cell_volume_field, "Thickness", -10*frame_thick)  # the mesh size is interpolated between VIn and VOut in a layer around the box of the prescribed thickness.
# # #         gmsh.model.mesh.field.setNumber(cell_volume_field, "VOut", frame_thick)
# # #         gmsh.model.mesh.field.setNumber(cell_volume_field, "VIn", cell_thick)
...
minimum = gmsh.model.mesh.field.add("Min")
gmsh.model.mesh.field.setNumbers(minimum, "FieldsList", box_field_list)  # take the minimum value of a list of fields.
gmsh.model.mesh.field.setAsBackgroundMesh(minimum)               # set the minimum field as the background mesh size field
...
# gmsh.write("panel_geo.inp")      # for ansys, abaqus
