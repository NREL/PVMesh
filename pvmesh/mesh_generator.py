    ...
    for key, data in domain_markers.items():
        if isinstance(data, dict) and "gmsh_tags" in data:
            ...
            if data["entity"] == "facet":
                gmsh.model.addPhysicalGroup(ndim - 1, data["gmsh_tags"], data["idx"])
...
left_frame = gmsh.model.occ.extrude(
    [(2, frame_surface_xz)], 0, panel_width + 2 * seal_length + 2 * frame_extended, 0
)
...
cell_front_glass_frag = gmsh.model.occ.fragment(
    [(3, uncovered_front_glass_frag[0][0][1])], cell_front_glass_list, removeTool=True
)
